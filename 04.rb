...
class IsThisRealLife

  def initialize(input)
    @encrypted_names = input.split()
  end

  def get_real_room_sector_id_sum
    real_room_sector_id_sum = 0
    ...
    @encrypted_names.each do |encrypted_name|
      ...
      matches = last_part.match(/^(.+)\[(.+)\]$/)
      sector_id = matches[1].to_i
      actual_checksum = matches[2]

      freq = Hash.new(0)
      name_part.scan(/\w/) {|char| freq[char] += 1}
      calculated_checksum = freq.sort_by { |char, count| [count * -1, char] }[0..4].map { |pair| pair.first }.join()

      real_room_sector_id_sum += sector_id if calculated_checksum == actual_checksum
    end

    real_room_sector_id_sum
  end

  def get_north_pole_storage_sector_id
    north_pole_storage_sector_id = 0

    @encrypted_names.each do |encrypted_name|
      parts = encrypted_name.split('-')
      name_parts = parts[0..-2]
      last_part = parts[-1]
      ...
      deciphered_name_parts = name_parts.map do |name_part|
        deciphered_name_part = ''
        name_part.each_char do |char|

          shift_amt.times { char = (char == 'z') ? 'a' : char.next }

          deciphered_name_part += char
        end

        deciphered_name_part
      end

      deciphered_name = deciphered_name_parts.join(' ')

      return sector_id if deciphered_name == 'northpole object storage'
    end
  end

end

itrl = IsThisRealLife.new(INPUT)
puts "Sum of the sector IDs of real rooms is #{itrl.get_real_room_sector_id_sum()}"
puts "Shift cipher results: #{itrl.get_north_pole_storage_sector_id()}"
